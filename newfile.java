...
class GameManager{
            ...
            for(Pacman p_curr: myPacmans){
                ...
                if(locked)
                {
                    for(Pacman p2: myPacmans){                     
                        if(p_curr.pacId!=p2.pacId && p_curr.isNear(p2,1) && p2.locked)
                        {
                    ...
                    p_curr.changeDirection(p_curr.choice.direction);
    ...
    void goAway(Pacman p, Pacman near){
        ...
        if(p.choice!=null)
            System.err.println(" e  nuova choice "+p.choice.direction);
    }
            ...
            if(p.typeId.equals("DEAD")==false){
            	...
            	if(p.action.equals("MOVE")) {
                    ...
                    if(p.choice==null){
                        ...
                        temp+=Integer.toString(p.x) + " " + Integer.toString(newY) + " "+Integer.toString(p.x)+"-"+Integer.toString(newY);
                    }
                        ...
                        temp+=Integer.toString(newX) + " " + Integer.toString((p.y)) + " "+Integer.toString(newX)+"-"+Integer.toString(p.y);
                    }
